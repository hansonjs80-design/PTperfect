...
const SPECIAL_NOTE_DESKTOP_DEFAULT_WIDTH = 150;
...
const applyDefaultWidthProfile = (widths: number[]) => {
  ...
  if (isDesktop && next[SPECIAL_NOTE_COL_INDEX] > 0) {
    next[SPECIAL_NOTE_COL_INDEX] = clampWidthByIndex(SPECIAL_NOTE_DESKTOP_DEFAULT_WIDTH, SPECIAL_NOTE_COL_INDEX);
  }
