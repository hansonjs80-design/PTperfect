...
export const usePatientBedSync = (
  ...
  const hasExerciseKeyword = useCallback((treatmentName: string) => {
    const text = treatmentName.toLowerCase();
    return text.includes('운동') || text.includes('exercise');
  }, []);
  ...
  const updateVisitWithBedSync = useCallback(async (id: string, updates: Partial<PatientVisit>, skipBedSync: boolean = false) => {
    ...
    const normalizedUpdates: Partial<PatientVisit> = { ...updates };
    if (
      typeof normalizedUpdates.treatment_name === 'string'
      && normalizedUpdates.is_exercise === undefined
      && hasExerciseKeyword(normalizedUpdates.treatment_name)
      && !oldVisit.is_exercise
    ) {
      normalizedUpdates.is_exercise = true;
    }

    if (!hasMeaningfulUpdates(oldVisit, normalizedUpdates)) return;

    let shouldForceRestart = false;

    // Conflict Check 1: Bed Assignment Change
    const targetBedId = normalizedUpdates.bed_id !== undefined ? normalizedUpdates.bed_id : oldVisit.bed_id;

    if (!skipBedSync && targetBedId) {
      // Case A: Moving/Assigning Bed
      const isBedAssignmentChange = normalizedUpdates.bed_id !== undefined && normalizedUpdates.bed_id !== oldVisit.bed_id;
    ...
    await updateLogVisit(id, normalizedUpdates);
    ...
    const mergedVisit = { ...oldVisit, ...(latestVisit || {}), ...normalizedUpdates };
    if (mergedVisit.visit_date && mergedVisit.visit_date !== currentDate) return;

    if (mergedVisit.bed_id && normalizedUpdates.memo !== undefined) {
      updateBedMemoFromLog(mergedVisit.bed_id, normalizedUpdates.memo || undefined);
    }

    if (mergedVisit.bed_id) {
      const hasStatusFlagUpdates = (
        normalizedUpdates.is_injection !== undefined ||
        normalizedUpdates.is_fluid !== undefined ||
        normalizedUpdates.is_traction !== undefined ||
        normalizedUpdates.is_eswt !== undefined ||
        normalizedUpdates.is_manual !== undefined ||
        normalizedUpdates.is_ion !== undefined ||
        normalizedUpdates.is_exercise !== undefined
      );

      if (hasStatusFlagUpdates) {
        // 1) 배드 카드 상태는 즉시 반영 (활성 배드일 때)
        updateBedFlagsFromLog(mergedVisit.bed_id, normalizedUpdates);
        ...
        if (latestVisitForBed && latestVisitForBed.id !== id) {
          await updateLogVisit(latestVisitForBed.id, normalizedUpdates);
        }
      }

      // 처방명 변경은 배드카드/최신 로그행 기준을 일치시켜 실시간 동기화 안정화
      if (normalizedUpdates.treatment_name !== undefined) {
        const latestVisitForBed = getLatestVisitForBed(mergedVisit.bed_id);
        if (latestVisitForBed && latestVisitForBed.id !== id) {
          const treatmentMirrorUpdates: Partial<PatientVisit> = { treatment_name: normalizedUpdates.treatment_name };
          if (normalizedUpdates.is_exercise !== undefined) {
            treatmentMirrorUpdates.is_exercise = normalizedUpdates.is_exercise;
          }
          await updateLogVisit(latestVisitForBed.id, treatmentMirrorUpdates);
        }
      }
    }

    const shouldApplyBedRuntimeSync = normalizedUpdates.bed_id !== undefined || normalizedUpdates.treatment_name !== undefined;
    // 이름/부위/처방/메모/상태/작성 등 로그 편집은 배드 활성화/타이머 로직에 영향 주지 않음.
    if (!shouldApplyBedRuntimeSync) {
      return;
    }

    if (oldVisit.bed_id && normalizedUpdates.bed_id === null) {
    ...
    if (mergedVisit.bed_id) {
      ...
      const isDirectBedAssignment = normalizedUpdates.bed_id !== undefined && normalizedUpdates.bed_id === mergedVisit.bed_id;
      if (!isLatestVisitForBed(id, mergedVisit.bed_id) && !isDirectBedAssignment) {
        return;
      }

      if (oldVisit.bed_id && normalizedUpdates.bed_id && oldVisit.bed_id !== normalizedUpdates.bed_id) {
  ...
  }, [updateLogVisit, clearBed, overrideBedFromLog, updateBedMemoFromLog, updateBedFlagsFromLog, bedsRef, visitsRef, getLatestVisitForBed, isLatestVisitForBed, hasMeaningfulUpdates, isTodayMode, currentDate, hasExerciseKeyword]);
