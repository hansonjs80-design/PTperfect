...
export const AuthorSelectorCell: React.FC<AuthorSelectorCellProps> = ({
  ...
  return (
    <>
      <div
        ref={cellRef}
        className={`absolute inset-0 w-full min-w-0 h-full min-h-[36px] px-2 grid place-items-center text-center cursor-pointer hover:bg-gray-50 dark:hover:bg-slate-700/50 transition-colors group outline-none focus:outline focus:outline-2 focus:outline-sky-400 focus:outline-offset-[-1px] focus:z-10 ${isDraft ? 'opacity-50 hover:opacity-100' : ''}`}
