...
export const PatientLogTableHeader: React.FC<PatientLogTableHeaderProps> = ({
  ...
  const handle = (colIndex: number) => {
    ...
    return (
      <div
        className="absolute top-0 right-[-5px] w-[12px] h-full z-20 flex items-center justify-center cursor-col-resize touch-none"
        onMouseDown={(e) => {
          e.preventDefault();
  ...
  return (
    <thead className="sticky top-0 bg-sky-100/95 dark:bg-slate-700/95 z-10 shadow-sm border-b-2 border-sky-200 dark:border-slate-500 backdrop-blur-md">
      <tr>
        ...
        <th className={`${thBase} w-[52px] min-w-[52px] max-w-[52px] relative whitespace-nowrap`}>
          삭제
        </th>
      </tr>
    </thead>
