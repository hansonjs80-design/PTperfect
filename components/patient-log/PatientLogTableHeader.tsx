...
export const PatientLogTableHeader: React.FC<PatientLogTableHeaderProps> = ({
  ...
  const handle = (colIndex: number, side: 'right' | 'left' = 'right') => {
    ...
    return (
      <div
        className={`absolute top-0 ${side === 'right' ? 'right-[-7px]' : 'left-[-7px]'} w-[16px] h-full z-20 flex items-center justify-center cursor-col-resize touch-none`}
        onMouseDown={(e) => {
          e.preventDefault();
  ...
  return (
    <thead className="sticky top-0 bg-sky-100/95 dark:bg-slate-700/95 z-10 shadow-sm border-b-2 border-sky-200 dark:border-slate-500 backdrop-blur-md">
      <tr>
        ...
        <th className={`${thBase} min-w-[96px] relative whitespace-nowrap`}>
          작성
          {handle(9)}
        </th>
        <th className={`${thBase} min-w-[52px] relative whitespace-nowrap border-l border-slate-200 dark:border-slate-700`}>
          삭제
          {handle(9, 'left')}
        </th>
      </tr>
