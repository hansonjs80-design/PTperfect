...
import { Syringe, Hand, Zap, ArrowUpFromLine, Droplet, Star, Atom, Dumbbell } from 'lucide-react';
...
export const PatientStatusIcons: React.FC<PatientStatusIconsProps> = ({ visit }) => {
  ...
  const hasStatus = visit.is_injection || visit.is_fluid || visit.is_manual || visit.is_eswt || visit.is_traction || visit.is_ion || visit.is_exercise || !!visit.memo;
  ...
  return (
    ...
    <div className="flex items-center justify-center flex-wrap gap-1 w-full h-full select-none py-1">
      ...
      {visit.is_exercise && (
        <div title="운동" className="flex">
          <Dumbbell className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-lime-500" strokeWidth={2.2} />
        </div>
      )}
      {visit.memo && (
