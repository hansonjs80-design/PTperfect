...
export const PatientLogRow: React.FC<PatientLogRowProps> = memo(({
  ...
  const buildOrderInsensitiveSignature = (steps: TreatmentStep[]) => {
    return [...steps]
      .map((step) => `${normalizeTreatmentString(step.name || '')}|${Number(step.duration ?? 0)}`)
      .sort()
      .join('||');
  };
  const isActivePresetModified = (() => {
    ...
    // 순서만 바뀐 경우는 수정으로 보지 않는다.
    // 항목 추가/삭제/교체(이름/시간 변경 포함)일 때만 노란색 수정 표시를 한다.
    const currentSteps = parseTreatmentString(treatmentDisplayValue, quickTreatments);
    const currentSignature = buildOrderInsensitiveSignature(currentSteps);
    const baseSignature = buildOrderInsensitiveSignature(activeBasePreset.steps);
    return !!currentSignature && currentSignature !== baseSignature;
  })();
