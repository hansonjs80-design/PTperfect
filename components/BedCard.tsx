...
export const BedCard: React.FC<BedCardProps> = memo(({
  ...
  const containerClass = useMemo(() => getBedCardStyles(bed, isOvertime, isNearEnd), [
    bed.status, bed.isInjection, bed.isFluid, bed.isESWT, bed.isTraction, bed.isManual, bed.isIon, isOvertime, isNearEnd
  ]);
  ...
  const responsiveCardHeightClass = 'md:h-[95%] md:min-h-0 md:self-start xl:h-[77%]';

  return (
    <div className={`${containerClass} ${responsiveCardHeightClass} transform transition-transform duration-200 active:scale-[0.99]`}>
      <BedHeader
        bed={bed}
      ...
      {/* Main Content Area */}
      <div className="flex-none sm:flex-1 flex flex-col w-full min-h-0 relative bg-white/40 dark:bg-slate-800/20 backdrop-blur-xs">
        <div className="flex-none h-auto sm:flex-1 sm:h-full sm:landscape:flex-none sm:landscape:h-auto lg:landscape:flex-1 lg:landscape:h-full flex flex-row w-full min-h-0">
          {bed.status === BedStatus.IDLE ? (
            ...
            <div
              className="w-full h-auto min-h-0 sm:h-full"
  ...
  return (
    ...
    prevProps.bed.isIon === nextProps.bed.isIon &&
    prevProps.bed.isExercise === nextProps.bed.isExercise &&
