...
export const BedEmptyState: React.FC<BedEmptyStateProps> = ({ bedId, onOpenSelector, onStartTimerOnly }) => {
  ...
  return (
    <div
      onClick={handleClick}
      className="w-full h-full relative overflow-hidden flex flex-col items-center justify-center cursor-pointer group select-none touch-manipulation"
    >
      ...
      <div className="absolute bottom-1.5 sm:bottom-2 left-1/2 -translate-x-1/2 scale-[0.92] sm:scale-100 origin-bottom" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center gap-1.5 sm:gap-2 text-[10px] sm:text-[11px] font-bold text-slate-500 dark:text-slate-300 bg-white/85 dark:bg-slate-800/85 px-2 py-1 rounded-lg border border-slate-200 dark:border-slate-700 shadow-sm">
          ...
          <div className="flex items-center gap-1">
            <input
              ...
              className="w-[56px] sm:w-[62px] px-1 py-1 text-[10px] sm:text-[11px] text-center font-bold rounded border border-brand-300 bg-white dark:bg-slate-800 dark:border-slate-600"
              aria-label="타이머 분 설정"
            />
