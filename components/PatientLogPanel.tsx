...
export const PatientLogPanel: React.FC<PatientLogPanelProps> = ({ onClose }) => {
  ...
  const undoLogOnly = useCallback(async () => {
    ...
    try {
      ...
      // 처방 목록 되돌리기 시 활성 배드와의 동기화가 어긋나지 않도록
      // 변경된 행의 bed/treatment 값을 다시 한번 동기화한다.
      const currentById = new Map(visits.map((v) => [v.id, v]));
      for (const nextVisit of prev) {
        const oldVisit = currentById.get(nextVisit.id);
        const treatmentChanged = (oldVisit?.treatment_name || '') !== (nextVisit.treatment_name || '');
        const bedChanged = (oldVisit?.bed_id || null) !== (nextVisit.bed_id || null);
        if (!treatmentChanged && !bedChanged) continue;

        await updateVisitWithBedSync(nextVisit.id, {
          bed_id: nextVisit.bed_id,
          treatment_name: nextVisit.treatment_name || '',
          memo: nextVisit.memo || '',
          is_injection: nextVisit.is_injection || false,
          is_fluid: nextVisit.is_fluid || false,
          is_traction: nextVisit.is_traction || false,
          is_eswt: nextVisit.is_eswt || false,
          is_manual: nextVisit.is_manual || false,
          is_ion: nextVisit.is_ion || false,
          is_exercise: nextVisit.is_exercise || false,
        }, false);
      }
    } catch (error) {
  ...
  }, [cloneVisits, setVisits, syncSnapshotToDb, visits, updateVisitWithBedSync]);
