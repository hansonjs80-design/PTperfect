...
export const mapRowToBed = (row: any): Partial<BedState> => {
  ...
  if (row.is_ion !== undefined) result.isIon = !!row.is_ion;
  if (row.is_exercise !== undefined) result.isExercise = !!row.is_exercise;
...
export const mapBedToDbPayload = (updates: Partial<BedState>): any => {
  ...
  if (updates.isIon !== undefined) payload.is_ion = updates.isIon;
  if (updates.isExercise !== undefined) payload.is_exercise = updates.isExercise;
