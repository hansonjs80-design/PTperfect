...
begin 
  ...
  if not exists (select 1 from information_schema.columns where table_name = 'beds' and column_name = 'is_ion') then
    alter table public.beds add column is_ion boolean default false;
  end if;
  if not exists (select 1 from information_schema.columns where table_name = 'beds' and column_name = 'is_exercise') then
    alter table public.beds add column is_exercise boolean default false;
  end if;
...
begin 
  ...
  if not exists (select 1 from information_schema.columns where table_name = 'patient_visits' and column_name = 'gender') then
    alter table public.patient_visits add column gender text;
  end if;
  ...
  if not exists (select 1 from information_schema.columns where table_name = 'patient_visits' and column_name = 'special_note') then
    alter table public.patient_visits add column special_note text;
  end if;
end $$;
